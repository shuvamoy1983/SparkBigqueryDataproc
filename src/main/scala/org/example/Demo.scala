...
object Demo {
  ...
  val spark = SparkSession.builder().appName("WriteToBigquery").config(conf).getOrCreate()
  ...
  def demo1() ={
    ...
    val Schema = new String(
      Files.readAllBytes(Paths.get("gs://mysrcfile/sam1.avsc")))

    val personDF1 = df1.select(from_avro(col("value"), Schema).as("person1"))
      .select("person1.*")

    personDF1.writeStream.outputMode("append").format("csv")
      .option("path", "gs://mytab1/demo1/")
